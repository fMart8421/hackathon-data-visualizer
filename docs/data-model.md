...
**Reviewed 2026-08-14** (OPEN-14 closed). The prototype is a single page with one tab per sensor domain: GNSS, Volatiles, Magnetometer, Accel/Gyro, UV Radiation, Meteorology. Each tab puts the primary visual on the left at about two thirds width and a narrow right column holding a "current reading" card, one large value over three or four small supporting fields, above a session playback control.

What was adopted:

| Prototype | Here |
|---|---|
| Tab per sensor domain | One dashboard per domain, plus a tag-driven link bar on every dashboard that behaves as the tab row. Domains match, except GNSS and Volatiles each split in two because their captures are months apart |
| Concentration coloured onto a map with a gradient scale and min/max labels | `particulates`, exactly this |
| Meteorology as Temp/Humidity/Pressure layers over a map | **Not possible.** The rigs that measured temperature and humidity had no GPS, and nothing measured pressure. `weather` is a time series and says so on the dashboard |
| Session playback scrubber | No Grafana equivalent without a plugin. The time picker and auto-refresh are the analogue |
| Right-hand "current reading" column | Kept Grafana's idiom: a row of stat panels along the top. Same information, standard placement |

Designs for the phase 6 domains, recorded now because they are the specification for those panels:

- **Magnetometer:** arrow markers on a map, arrow length proportional to field magnitude and orientation showing the horizontal vector (Bx, By); a "current vector" card with total µT over Bx/By/Bz. A world declination model was also shown, which is reference data rather than telemetry and is out of scope.
- **Accel/Gyro:** two panels, accelerometer and gyroscope, each with X, Y and Z as separate stacked traces and the current value printed at the right of each trace. A transport bar above with play, reverse, zoom and range.
- **UV Radiation:** a current-index gauge over a 0 to 12+ colour band, beside a 24-hour bar profile by hour. A global erythemal map was also shown; again reference data, out of scope.
...
**Phase 5, dashboards for measured data. Done.** Six dashboards, all provisioned automatically and all verified rendering in a browser, not just by query: `gnss-rtk` and `gnss-survey` (1), `volatiles` and `particulates` (2), `weather` (6, temperature and humidity only), `metric-explorer` (7). Requirement 9 is structural, already in place since phase 1, confirmed still true against real data.

**One dashboard per capture epoch.** The first attempt put GNSS on one dashboard and air quality on another, which cannot work: a Grafana time range is dashboard-wide, and the RTK captures (Nov 2025) are five months from the survey occupations (Apr 2026), while the gas campaign (May 2026) is eleven months from the geo-referenced bike rides (Jun 2025). Any range wide enough to include both compressed each into an unreadable spike. Each dashboard now pins an absolute range around the data it shows.
